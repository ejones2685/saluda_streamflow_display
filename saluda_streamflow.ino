...
const char* url = "https://waterservices.usgs.gov/nwis/iv/?format=json&sites=02169000&parameterCd=00060,00065&siteType=ST&siteStatus=all";
...
void RiverData(void) 
{
  ...
  HTTPClient http;
  http.begin(url);
  int httpResponseCode = http.GET();sa
  ...
  if (httpResponseCode == 200) 
  {
    ...
    float flow = doc["value"]["timeSeries"][0]["values"][0]["value"][0]["_value"];
    Serial.println(flow);
    ...
    Heltec.display->setFont(ArialMT_Plain_16);
    Heltec.display->drawString(85, 30, "ft3/s");
